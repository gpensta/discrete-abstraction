function G2graphviz(G, grid_step, x_inf, y_inf, scc, filename)
n = size(G, 1);
w = sqrt(n);
dotStr = 'digraph G {\n';
colors = [
    ...
    for j = 1:n
        if G(i, j) == 1
            ...
            if isIInSCC && isJInSCC
                dotStr = [dotStr, nodeId_i, ' -> ', nodeId_j, ';\n'];
            end
            ...
            if ~ismember(nodeId_i, nodesAdded)
                ...
                if isIInSCC
                    dotStr = [dotStr, nodeId_i, ' [label="', positionStr_i_float, '", pos="', positionStr_i, '!", style=filled, fillcolor="', char(colors(mod(ri, 10) +1)), '"];\n'];
                    nodesAdded{end+1} = nodeId_i;
                else
                    %dotStr = [dotStr, nodeId_i, ' [label="', labelStr_i,
                    %'", pos="', positionStr_i, '!"];\n']; % 
                end
            end
            ...
            if ~ismember(nodeId_j, nodesAdded)
                ...
                if isJInSCC
                    ...
                    dotStr = [dotStr, nodeId_j, ' [label="', positionStr_j_float, '", pos="', positionStr_j, '!", style=filled, fillcolor="', char(colors(mod(rj, 10) +1)), '"];\n'];
                    
                    nodesAdded{end+1} = nodeId_j;
                else
                    %dotStr = [dotStr, nodeId_j, ' [label="', labelStr_j, '", pos="', positionStr_j, '!"];\n'];
                end
